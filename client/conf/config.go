...
//go:generate mockgen -destination=../mocks/mock_configure.go -package=mocks github.com/calypr/data-client/client/conf ManagerInterface
...
func NewConfigure(cred *Credential, logs logs.Logger) ManagerInterface {
	return &Manager{
		Logger: logs,
	}
}
...
func (man *Manager) Import(filePath, fenceToken string) (*Credential, error) {
	...
	if filePath != "" {
		...
		if err != nil {
			man.Logger.Println("error parsing credential file path: " + err.Error())
			return nil, err
		}
		...
		if err != nil {
			if os.IsNotExist(err) {
				man.Logger.Println("File not found: " + fullPath)
			} else {
				man.Logger.Println("error reading file: " + err.Error())
			}
			return nil, err
		}
		...
		if err := json.Unmarshal([]byte(jsonStr), &cred); err != nil {
			...
			man.Logger.Println(errMsg.Error())
			return nil, errMsg
		}
	} else if fenceToken != "" {
